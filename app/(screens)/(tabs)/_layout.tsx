  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const idCustomer = decodedToken?.data.idCustomer // Lấy idCustomer từ token
        console.log(idCustomer)

        if (idCustomer) {
          const response = await getProfileCustomer(idCustomer); // Gọi API lấy thông tin người dùng
          ...
          <View style={{ width: '100%', paddingVertical: 30, paddingHorizontal: 20, borderBottomColor: '#F5F4F4', borderBottomWidth: 1 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 10 }}>
              ...
              <View style={{ paddingTop: 10 }}>
                <Text style={{ fontSize: 20, fontWeight: 'bold', color: 'black' }}>{user ? user.fullName : '' }</Text>
                <Text style={{ fontSize: 14, fontWeight: '400', color: 'black' }}>{user ? user.email : ''}</Text>
              </View>
            </View>
          </View>
  ...
  return (
    ...
    >
      ...
      */}
      ...
      <Drawer.Screen name="cart" component={Bills}
        options={{
          headerShown: true,
          headerTitle: "Carts",
          ...
          drawerLabel: ({ focused, color }) => {
            return (
              <View style={{ flex: 1 }}>
                <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Text style={{ flex: 1, color: focused ? "green" : color, fontSize: 16, fontWeight: 500, paddingLeft: 10 }}>Bills</Text>
                  <Entypo name="chevron-right" size={16} color={color} />
